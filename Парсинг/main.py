...
class DocumentTimeoutError(Exception):
    """Обработка одного документа заняла слишком много времени."""


@contextmanager
def time_limit(seconds: int):
    """
    Жёсткий таймаут на блок кода через signal.alarm (Unix/macOS — работает
    в этом окружении). Нужен, потому что pdfplumber иногда очень надолго
    (иногда практически бесконечно) зависает на поиске таблиц в сложных
    или больших PDF — без таймаута один такой файл мог бы остановить весь
    прогон на часы.
    """
    def _handler(signum, frame):
        raise DocumentTimeoutError(f"Превышен лимит {seconds}с")
...
def run(skip_yadisk: bool, skip_llm: bool, budget_rub: float, limit: int | None,
    ...
    if skip_yadisk:
        ...
        remote_files = yadisk_sync.sync_all(max_files=yadisk_limit)
    ...
    for idx, bundle in enumerate(bundles, start=1):
        logger.info(
            "[%d/%d] Обрабатываю: %s",
            idx, total, bundle.main_file or bundle.bundle_id,
        )
        try:
            with time_limit(config.MAX_SECONDS_PER_DOCUMENT):
                docs = process_bundle(bundle, skip_llm=skip_llm, budget=budget)
            # Пишем на диск СРАЗУ, а не в конце всего прогона — иначе при
            # сбое/прерывании многочасового прогона результат пропадёт
            # целиком, несмотря на то что документы уже были обработаны.
            for doc in docs:
                write_json(doc)
                processed_count += 1
        except DocumentTimeoutError as e:
            logger.error(
                "Таймаут (>%ds) на документе %s: %s. Пропускаю и иду дальше.",
                config.MAX_SECONDS_PER_DOCUMENT, bundle.main_file, e,
            )
        except BudgetExceededError as e:
            logger.warning("%s Останавливаю обработку новых документов.", e)
            stopped_by_budget = True
            break
        except Exception as e:
            logger.exception("Ошибка обработки bundle %s: %s", bundle.bundle_id, e)
